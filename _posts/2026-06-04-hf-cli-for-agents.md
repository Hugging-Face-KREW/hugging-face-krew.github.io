...
`hf` CLI는 수년 동안 주로 사용자를 위해 개발되어 왔습니다. 하지만 이제는 Claude Code, Codex, Cursor 등의 **코딩 에이전트**도 점점 더 많이 사용하고 있습니다. 그래서 두 사용자층 모두가 사용할 수 있도록 `hf` CLI를 다시 구축했습니다. 이 글에서는 무엇을 바꾸었고 어떻게 벤치마크했는지 정리합니다. 복잡한 다단계 작업에서는 에이전트가 `curl` 요청을 직접 작성하거나 Python SDK를 사용하는 비-CLI 기준선이 `hf` CLI보다 최대 **6배 더 많은 토큰**을 사용한다는 사실을 확인했습니다.
...
사람과 코딩 에이전트는 같은 `hf` 명령에서도 서로 다른 출력 형식을 기대합니다. 사람은 ANSI 색상, 터미널 너비에 맞게 줄인 표, 성공을 나타내는 녹색 ✅, 불리언 값 `✔`, 진행 표시줄, 설명형 힌트 등을 원합니다. 반대로 에이전트는 ANSI 없이 모든 값을 생략하지 않고 표시하며, 사람보다 훨씬 밀도 높은 출력도 처리할 수 있으므로 토큰 수를 줄일 수 있도록 간결하고 구조화된 형식을 원합니다. 또한 CLI 프롬프트에 응답할 수 없으며, 시간 초과가 발생하면 명령을 다시 실행할 수 있습니다. 이 절의 나머지 부분에서는 `hf`가 양쪽에 필요한 출력을 어떻게 제공하는지 설명합니다. 에이전트 모드 출력은 `hf` v1.9.0에서 도입했으며, 이후 릴리스에서 CLI의 나머지 부분도 차례로 이 방식으로 전환해 왔습니다.
